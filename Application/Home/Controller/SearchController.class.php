...
class SearchController extends Controller {
    public function index(){
        ...
        if($key == NULL){
            ...
            //先计算出一共有多少页
            $data_index_allnum = $db_index_booklist->where("(`title` LIKE '%".$key."%' OR `dec` LIKE '%".$key."%') AND `status` = 0")->order("time desc")->count();
            ...
            $data_index_booklist = $db_index_booklist->where("(`title` LIKE '%".$key."%' OR `dec` LIKE '%".$key."%') AND `status` = 0")->page($page,8)->order("time DESC")->select();
        ...
        $this->assign("status_error",$status_error);
        $this->display();
    }
}
